...
def main() -> int:
    ...
    # 流水留痕（AGENTS.md § 5.3）；失败不影响实验块本身
    try:
        import log_activity

        log_activity.append("Agent", "op", f"新建实验块 {exp_id}：{vibe}", [exp_id])
    except Exception as exc:
        print(f"[warn] 未能写入流水（{exc}）", file=sys.stderr)

    print("[next] 跑完填完字段；若结果改变了主线判断，追加时间线节点："
          f'`python3 tools/timeline.py add --kind experiment --topic <主题键> "一句话" --ref {exp_id}`')
    return 0

...
- initialized          -> mode + active issue + current week LOGS + this week's
                          activity trail (§ 5.3) + timeline / circling warnings
                          (§ 12) + ELI5 sync (§ 4.3) + retro due (§ 6)

This module is also the shared base the other protocol scripts import
(REPO / LOGS_DIR / now_cn / read / week helpers) — one implementation, many
callers. Optional imports of timeline / eli5_sync happen inside functions so a
repo missing those files still gets a briefing.
...
# 全仓库时间戳统一 UTC+8（bootstrap.md / AGENTS.md § 5.3）
...
def read(path: Path) -> str:
    try:
        ...
        # 有人用 GBK 存了中文文件时，宁可有损读取也不要让唯一的自检脚本崩掉。
...
def current_week() -> tuple[int, int]:
    return now_cn().date().isocalendar()[:2]
...
def activity_lines(now: tuple[int, int]) -> list[str]:
    """本周流水（§ 5.3）：有几条 + 最近 3 条，帮 Agent 接上上一次会话的上下文。"""
    week = week_str(now)
    path = LOGS_DIR / f"{week}-activity.md"
    if not path.exists():
        return [
            f"- 本周流水：LOGS/{week}-activity.md 尚未创建 → 用户拍板 / Agent 实质操作后 "
            '`python3 tools/log_activity.py "…"`（AGENTS.md § 5.3）'
        ]
    entries = [l.strip() for l in read(path).splitlines() if l.lstrip().startswith("- 【")]
    lines = [f"- 本周流水：LOGS/{week}-activity.md（{len(entries)} 条）"]
...
def timeline_lines() -> list[str]:
    """主线时间线 + 绕圈告警（§ 12）。缺文件/缺脚本时静默降级。"""
    try:
        import timeline  # 函数内导入：timeline 反过来 import 本模块
    except Exception:
        return []
    try:
        return timeline.briefing_lines()
    except Exception:
        return []


def eli5_lines() -> list[str]:
...
def briefing() -> str:
    ...
    lines.extend(activity_lines(now))
    lines.extend(timeline_lines())
    lines.extend(eli5_lines())
